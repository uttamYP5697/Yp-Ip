...
const Page = () => {
  ...
  console.log("🚀 ~ Page ~ openIndex:", openIndex)
  const [isOpen, setIsOpen] = useState(false); // Track open block
  ...
  // };
  const toggleBlockList = (index) => {
    // If openIndex is null, show the inserter; otherwise toggle the block visibility
    setOpenIndex((prevIndex) => (prevIndex === null ? index : (prevIndex === index ? null : index)));
  };
  ...
  const addBlock = (type) => {
    ...
    setIsOpen(false); // Close the inserter after adding
  };
